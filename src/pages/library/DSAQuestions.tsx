...
const DSAQuestions = () => {
  ...
  const folderQuestions = useMemo(() => {
    ...
    return items
      .filter((item) => item.question_source === "dsa")
      .map((item) => dsaQuestions.find((q) => q.id === item.question_id))
  ...
  const filteredQuestions = useMemo(() => {
    ...
    if (viewMode === "folders" && selectedFolderId) {
      ...
      if (topicFilter !== "all") {
        questions = questions.filter((q) => q.topicId === topicFilter);
      }
      return questions;
    }

    let questions = getQuestionsByTopic(topicFilter);
    questions = getQuestionsByDifficulty(questions, difficultyFilter);
    questions = searchQuestions(questions, searchQuery);
    ...
    return questions;
  }, [topicFilter, difficultyFilter, searchQuery, viewMode, isSolved, isRevision, selectedFolderId, folderQuestions]);

  // Progress stats
  const progressStats = useMemo(() => {
    const allQs = dsaQuestions;
  ...
  const getQuestionDetails = useCallback((questionId: number) => {
    ...
    const topic = dsaTopics.find((t) => t.id === question.topicId);
    return {
      id: question.id,
      text: question.title,
      difficulty: question.difficulty,
      categoryId: "dsa",
      categoryName: topic?.name || "General",
    };
  }, []);
  ...
  const hasActiveFilters = searchQuery.trim() !== "" || difficultyFilter !== "all" || topicFilter !== "all";
          ...
          <div className="flex h-16 items-center gap-4 px-4 md:px-6">
            <div className="flex items-center gap-3 flex-1 min-w-0">
              ...
              <div className="min-w-0">
                <h1 className="text-lg md:text-xl font-bold truncate">Data Structures & Algorithms</h1>
        ...
        <main className="p-4 md:p-6 lg:p-8 space-y-4 md:space-y-6 max-w-full overflow-x-hidden">
          ...
          >
            <TabsList>
              ...
              <TabsTrigger value="problem-sets">Problem Sets</TabsTrigger>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="hover:bg-transparent bg-muted/20">
                      ...
                      <TableHead className="hidden sm:table-cell w-24 text-xs font-semibold text-right">Difficulty</TableHead>
...
// DSA Question Row Component
interface DSAQuestionRowProps {
    ...
    <>
      ...
      <AnimatePresence>
        {isExpanded && question.answer && (
          <TableRow>
            <TableCell colSpan={hasFolders && isLoggedIn ? 6 : 5} className="p-0 border-0">
              <motion.div
...
const EmptyFolderState = () => (
  ...
  >
    ...
    <p className="text-sm text-muted-foreground mt-1">Add questions to this folder from the All problems tab.</p>
  </motion.div>
);
