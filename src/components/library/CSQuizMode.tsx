 ...
 import { type CSQuestion, csSubjects } from "@/data/csSubjectsData";
 ...
 const QUIZ_PRESETS: QuizPreset[] = [
   {
     id: "cs-easy-starter",
     name: "Easy Starter",
     description: "10 easy CS problems to build confidence",
     icon: <Sparkles className="h-5 w-5" />,
     config: { questionCount: 10, timePerQuestion: 60, subject: "all", difficulty: "Easy" },
     color: "from-emerald-500/20 to-green-500/20 border-emerald-500/30",
   },
   {
     id: "cs-medium-challenge",
     name: "Medium Challenge",
     description: "10 medium problems, balanced difficulty",
     icon: <Target className="h-5 w-5" />,
     config: { questionCount: 10, timePerQuestion: 90, subject: "all", difficulty: "Medium" },
   ...
   },
   {
     ...
     config: { questionCount: 10, timePerQuestion: 120, subject: "all", difficulty: "Hard" },
     color: "from-red-500/20 to-rose-500/20 border-red-500/30",
   },
   {
     id: "cs-os-focus",
     name: "OS Focus",
     description: "10 Operating System questions",
     icon: <Monitor className="h-5 w-5" />,
     config: { questionCount: 10, timePerQuestion: 90, subject: "os", difficulty: "all" },
     color: "from-purple-500/20 to-pink-500/20 border-purple-500/30",
   },
   {
     id: "cs-dbms-focus",
     name: "DBMS Focus",
     description: "10 Database questions",
     icon: <Database className="h-5 w-5" />,
     config: { questionCount: 10, timePerQuestion: 90, subject: "dbms", difficulty: "all" },
     color: "from-blue-500/20 to-cyan-500/20 border-blue-500/30",
   },
   {
     id: "cs-cn-focus",
     name: "Networks Focus",
     description: "10 Computer Networks questions",
     icon: <Network className="h-5 w-5" />,
     config: { questionCount: 10, timePerQuestion: 90, subject: "cn", difficulty: "all" },
     color: "from-orange-500/20 to-amber-500/20 border-orange-500/30",
   },
 ];
 
 const CS_TIMED_CHALLENGES: QuizPreset[] = [
   {
     id: "cs-easy-sprint",
     name: "Easy Sprint",
     description: "10 easy CS problems, race against time!",
     icon: <Timer className="h-5 w-5" />,
     config: { questionCount: 10, timePerQuestion: 30, subject: "all", difficulty: "Easy" },
   ...
   },
   {
     ...
     config: { questionCount: 15, timePerQuestion: 45, subject: "all", difficulty: "Medium" },
   ...
   },
   {
     ...
     config: { questionCount: 20, timePerQuestion: 60, subject: "all", difficulty: "Hard" },
     color: "from-red-500/20 to-rose-500/20 border-red-500/30",
     isChallenge: true,
   },
 ];
 
 const CSQuizMode: React.FC<CSQuizModeProps> = ({ questions, onClose }) => {
   const { user } = useAuth();
   ...
   const [quizQuestions, setQuizQuestions] = useState<CSQuestion[]>([]);
   ...
   const prepareQuiz = useCallback(() => {
     ...
     if (config.subject !== "all") {
       filtered = filtered.filter((q) => q.subjectId === config.subject);
     }
   ...
   if (phase === "config") {
     return (
       <div className="space-y-6">
         ...
         <Card className="border-primary/20 bg-gradient-to-br from-primary/5 to-primary/10 backdrop-blur">
           ...
           <CardContent>
             ...
             <div className="grid gap-3 sm:grid-cols-3">
               {CS_TIMED_CHALLENGES.map((preset) => (
                 <button
                   key={preset.id}
         ...
         {user && <QuizLeaderboard quizType="cs" currentUserId={user.id} />}
         {user && challengeId && <QuizLeaderboard quizType="cs" currentUserId={user.id} challengeId={challengeId} />}
       </div>
     );
   }
     ...
     return (
       <Card className="border-primary/20 bg-card/80 backdrop-blur">
         ...
         <CardContent className="space-y-6">
           ...
           {showLeaderboard && user && <QuizLeaderboard quizType="cs" currentUserId={user.id} challengeId={challengeId} />}
       ...
       <CardContent className="space-y-6">
         <AnimatePresence mode="wait">
           <motion.div key={currentIndex} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
             ...
             <p className="text-sm text-muted-foreground">{currentQuestion?.text}</p>
