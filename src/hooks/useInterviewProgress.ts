...
interface UseInterviewProgressReturn {
...
export function useInterviewProgress(): UseInterviewProgressReturn {
    ...
    const fetchProgress = async () => {
      ...
      try {
        const { data, error } = await supabase
          ...
          .eq("user_id", user.id)
          .eq("company_id", "interview-questions")
          .eq("tab_id", "all");
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`interview-progress-${user.id}`)
      .on(
        "postgres_changes",
        {
          event: "*",
        ...
        (payload: RealtimePostgresChangesPayload<{
          ...
          // Only process interview progress
          if (newRecord?.company_id !== "interview-questions" && oldRecord?.company_id !== "interview-questions") {
            return;
          }
  ...
  const upsertProgress = async (itemId: number, updates: Partial<ProgressItem>) => {
    ...
    try {
      const { error } = await supabase.from("user_company_progress").upsert(
        {
          user_id: user.id,
          company_id: "interview-questions",
          tab_id: "all",
          item_id: itemId,
